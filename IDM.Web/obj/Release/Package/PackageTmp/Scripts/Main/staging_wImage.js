var StagingModule = (function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (sourceTable) {
                        ...
                        $.ajax({
                            url: AppUrls.getByJobAndAnalysisPartialAsync,
                            type: 'GET',
                            data: {
                                table: sourceTable,
                            ...
                            error: function (xhr, status, error) {
                                ToastrHelper.notification("error", "Error fetching staging data: " + error + "; Status: " + status, "Error");
                                reject(error);
                            }
                        });
                    } else {
    ...
    function approvalFunction(status) {
        ...
        // Define the async task for GlobalFormPrep to track
        const processRequestApproval = function () {
            return new Promise((resolve, reject) => {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (destinationTable) {
                            // Capture the full current URL including all query parameters
        ...
        // Trigger the loading overlay specifically for this action
        GlobalFormPrep.standardPage([
            processRequestApproval()
        ], {
            loadingMessage: `Applying ${status}...`, // Your custom message
            loadingSubtitle: 'Updating records and finalizing analysis data', // Your custom subtitle
            onAfterLoad: function (results) {
                ToastrHelper.notification("info", `Status ${status} applied successfully`, "Information");
                $('#approve-btn, #reject-btn').prop('disabled', true).addClass('disabled');
            },
            onError: function (error) {
                ToastrHelper.notification("error", "Action failed: " + error, "Error");
            }
        });
